  ...
  if (query.isPending)
    return (
      <p className="py-12 text-sm text-muted-foreground" role="status">
        {_copy('Loading your orders...')}
      </p>
    );
  if (query.error)
    return (
      <Alert
        title={_copy('Orders unavailable')}
    ...
    return (
      <div className="border border-dashed border-border bg-surface p-10 text-center">
        <h2 className="type-h4 text-primary">
          {_copy('You haven&apos;t placed any orders yet.')}
        </h2>
        <p className="mt-2 text-sm text-muted-foreground">
          {_copy(
            'Explore SANAD services and choose the support that fits your next step.',
          )}
        </p>
        ...
        <table className="w-full min-w-[760px] text-start text-sm">
          <thead className="bg-surface-muted text-xs tracking-wide text-secondary uppercase">
            <tr>
              ...
              <th className="px-5 py-4">{_copy('Payment')}</th>
              <th className="px-5 py-4">{_copy('Status')}</th>
              <th className="px-5 py-4">{_copy('Purchased')}</th>
              <th className="px-5 py-4">
                <span className="sr-only">{_copy('Actions')}</span>
              </th>
            </tr>
          </thead>
        ...
        {query.data.items.map((order) => (
          ...
          >
            ...
            <dl className="mt-4 grid grid-cols-2 gap-3 text-sm">
              ...
              <div>
                <dt className="text-muted-foreground">{_copy('Payment')}</dt>
                <dd className="mt-1">
                  {_copy(_copy.status(order.paymentStatus))}
                </dd>
              </div>
              <div className="col-span-2">
                <dt className="text-muted-foreground">{_copy('Purchased')}</dt>
                <dd className="mt-1">{_copy(_copy.date(order.createdAt))}</dd>
              </div>
            </dl>
      ...
      {query.data.meta.totalPages > 1 ? (
        <div className="mt-7 flex items-center justify-between gap-4">
          ...
          <p className="text-sm text-muted-foreground">
            {_copy('Page')}
            {_copy(page)} {_copy('of')}
            {_copy(query.data.meta.totalPages)}
          </p>
          <Button
